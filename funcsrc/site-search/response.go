...
func (s *SearchResponse) AddAggregations(aggs *search.Bucket, filters []*Filter) {
	...
	s.buildAggregation(aggs, "tags", filters)
	s.buildAggregation(aggs, "subjects", filters)
	s.buildAggregation(aggs, "creators", filters)
	s.buildAggregation(aggs, "venues", filters)
	s.buildAggregation(aggs, "sources", filters)
}
